...
UPDATE Employees
SET VentasTotales =(
	SELECT SUM(ISNULL(Orders.VentasTotales,0))
	FROM Orders
...
SET VentasTotales =(
	...
	FROM Orders
	WHERE Orders.CustomerID = CLIENTES.CODIGO
	)
	...
	BEGIN
		...
		SET VentasTotales = VentasTotales
			+ ISNULL(
				(SELECT VentasTotales
				FROM Inserted
				WHERE Inserted.CustomerID =Clientes.Codigo), 0)
		- ISNULL(
				(SELECT VentasTotales
				FROM Deleted
				WHERE Deleted.CustomerID = Clientes.Codigo), 0)
